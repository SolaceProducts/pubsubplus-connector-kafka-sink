...
public class SolaceSinkConnectorConfig extends AbstractConfig {
  ...
  public static ConfigDef solaceConfigDef() {
    return new ConfigDef()
        ...
        .define(
                SolaceSinkConstants.SOL_MESSAGE_DELIVERY_MODE,
                Type.STRING,
                null,
                ValidStringIfNoNull.in(Arrays.stream(DeliveryMode.values()).map(Enum::name).toArray(String[]::new)),
                Importance.HIGH,
                "The message delivery modes used for sending messages, i.e., PERSISTENT, NON_PERSISTENT or DIRECT. "
                      +  "If not specified, the connector will either default to PERSISTENT for sending messages to Solace queue "
                      +  "or to DIRECT for Solace Topic."
        );

  }

  public Optional<DeliveryMode> getMessageDeliveryMode() {
    return Optional.ofNullable(getString(SolaceSinkConstants.SOL_MESSAGE_DELIVERY_MODE)).map(DeliveryMode::valueOf);
  }

  static ConfigDef config = solaceConfigDef();

  public static class ValidStringIfNoNull implements ConfigDef.Validator {

    final List<String> validStrings;

    private ValidStringIfNoNull(final List<String> validStrings) {
      this.validStrings = validStrings;
    }

    public static ValidStringIfNoNull in(final String... validStrings) {
      return new ValidStringIfNoNull(Arrays.asList(validStrings));
    }
